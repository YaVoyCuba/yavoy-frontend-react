    ...
    <main className="-mx-3 bg-[#f6f4f5] text-slate-900 lg:-mx-14">
      <section className="mx-auto grid w-full max-w-7xl gap-10 px-4 py-12 lg:grid-cols-2 lg:px-8">
        <div className="stitch-reveal">
          <h1 className="text-5xl font-black leading-tight md:text-6xl"><Trans>Como podemos ayudarte?</Trans></h1>
          <p className="mt-4 max-w-lg text-lg leading-relaxed text-slate-600">
            <Trans>Estamos aqui para asistirte con tus viajes y envios a Cuba. Escribenos y nos pondremos en contacto contigo lo antes posible.</Trans>
          </p>
          ...
          <form onSubmit={handleSubmit} className="mt-7 rounded-xl border border-slate-200 bg-white p-6 shadow-sm md:p-7">
            <div className="grid gap-4 md:grid-cols-2">
              <label className="text-sm font-semibold text-slate-700">
                <Trans>Nombre</Trans>
                <input className="mt-2 w-full rounded-lg border-slate-200" placeholder={t`Nombre completo`} type="text" />
              </label>
              <label className="text-sm font-semibold text-slate-700">
                <Trans>Correo electronico</Trans>
                <input className="mt-2 w-full rounded-lg border-slate-200" placeholder={t`correo@ejemplo.com`} type="email" />
              </label>
            </div>
            <label className="mt-4 block text-sm font-semibold text-slate-700">
              <Trans>Asunto</Trans>
              <input className="mt-2 w-full rounded-lg border-slate-200" placeholder={t`Consulta sobre...`} type="text" />
            </label>
            <label className="mt-4 block text-sm font-semibold text-slate-700">
              <Trans>Mensaje</Trans>
              <textarea
                className="mt-2 min-h-[140px] w-full resize-none rounded-lg border-slate-200"
                placeholder={t`Cuentanos mas sobre tus necesidades...`}
              />
            </label>
        ...
        <div className="space-y-4 stitch-reveal" style={{ animationDelay: "120ms" }}>
          <article className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm md:p-7">
            ...
            <p className="text-sm text-slate-500"><Trans>Visitanos</Trans></p>
          ...
          <div className="relative h-[320px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <img src="/assets/img/fondo.webp" alt={t`Ubicacion en Miami`} className="h-full w-full object-cover grayscale" />
            <div className="absolute bottom-4 left-4 rounded-lg bg-white px-4 py-2 text-sm font-bold shadow-lg">
              <Trans>Nuestra sede en Miami</Trans>
            </div>
          </div>

          <div className="rounded-xl border border-[#f2cdc0] bg-[#fff1ec] p-5 md:p-6">
            <h3 className="font-bold text-[#f06233]"><Trans>Horario de atencion al cliente</Trans></h3>
            <div className="mt-3 grid grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-slate-500"><Trans>Lun - Vie</Trans></p>
                <p className="font-bold"><Trans>9:00 AM - 6:00 PM</Trans></p>
              </div>
              <div>
                <p className="text-slate-500"><Trans>Sabado</Trans></p>
